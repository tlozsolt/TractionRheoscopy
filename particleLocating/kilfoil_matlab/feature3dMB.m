function [r]=feature3dMB(image, diameter,masksz, xyzmax,inputv, sep, masscut, threshold,zeroPxCut)
...
a = ones( nx+extent(1), ny+extent(2), nz+extent(3),'single' );
...
extent = fix(masksz) + 1;
extent = extent + mod((extent+1),2);
xl = x - fix(extent(1)/2); % for positive input x, fix(x) is equivalent to floor(x)
xh = xl + extent(1) -1;
yl = y - fix(extent(2)/2); 
yh = yl + extent(2) -1;
zl = z - fix(extent(3)/2); 
...
nask  = sum(mask(:));
rmask = (rsq.*mask)+(1/6); % .* means element-wise multiplication of matrix or vector entries.   
...
countZeroPxTemp = zeros(1,nmax);
tops = zeros(1,nmax); % Zsolt, runtime errors gave undefined function/variable tops on line 192
for i=1:nmax
...
for i=1:nmax
    ...
    countZeroPxTmp(i) = sum(sum(sum(temp==0))); % This should return the number of zero pixels in the mask region around a candidate particle..but I think that mask is, at this point in the script, zero-padded for out of bounds errors and hence
    
end
%do a masscut, and prevent divide by zeroes in the centroid calc.
%figure; hist(m,[0:1e3:2e5]); pause(0.5)
%figure; hist(countZeroPxTmp,[0:2:250]); pause(0.5);
%figure; histogram2(m,countZeroPxTmp); pause(0.5);
masscut
%masscut = input('masscut: ')
%w = find( m > masscut); %only those features with a total mass higher than masscut are considered to be a real feature
w = find( m > masscut & countZeroPxTmp<zeroPxCut); %only those features with a total mass higher than masscut and number of id zero pixels less than 10 are considered to be a real feature

% At this point, you should also consider setting a parameter for number of
% identically zero pixels in the smaller search region...ths would capture
% overlap with zeroed out false particles during iterative locating
nmax=length(w);
if nmax==0 
disp('No features found!');
nmax=-1;
...
zc = zeros(1,nmax);
rg = zeros(1,nmax);
countZeroPx = zeros(1,nmax); % set up the vector to count zero pixels 
...
for j=1:20
  ...
  for i=1:nmax
    ...
    countZeroPx(i) = sum(sum(sum((suba(:,:,:,i))<0.01))); % This should return the number of zero pixels in the mask region around a candidate particle..but I think that mask is, at this point in the script, zero-padded for out of bounds errors and hence
  end
  for i=1:nmax
