...
%some clarification comments have been added by Kate Jensen Fall 2008 (no
%code changes) and are marked with KEJ + YYMMDD (KEJ 081104)
...
% do xdirection masks
bb=single(lnoise(1));
w = round( max(lobject(1),2 * bb) );  %this means that the filter sizes will be set by the larger of the noise and the object size. If the object size is larger than twice the noise value, then the noise value matters almost not at all. kej 081104
N = 2*w + 1;
r = single([-w:w])/(2 * bb); %so the filter is twice the size of the object -- meaning, I think, that lobject should be a radius! (no, diameter.)
gx = exp( -r.^2 );
gx = gx /sum(gx); %gx is a normalized gaussian filter - kej 081104
